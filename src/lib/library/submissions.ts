...
/** What kind of contribution a row is. Legacy rows (no column) are documents. */
export type SubmissionType = "document" | "url";
/** For URL submissions: crawl the whole site ("site") or just the page ("page"). */
export type CrawlMode = "site" | "page";

export interface LibrarySubmission {
  id: string;
  date_created?: string | null;
  /** "document" (file upload) or "url" (website crawled into Cortex). */
  submission_type?: SubmissionType;
  title?: string | null;
  /** Directus file id (uuid). Null for URL submissions. */
  file?: string | null;
  filename?: string | null;
  file_type?: string | null;
  file_size?: number | null;
  /** The submitted website URL (URL submissions only). */
  url?: string | null;
  /** URL submissions: "site" (crawl whole site) or "page" (just this URL). */
  crawl_mode?: CrawlMode | null;
  /** Lowercased, SIWE-verified submitter address. */
  submitted_by?: string | null;
  status?: SubmissionStatus;
  rejected_reason?: string | null;
  /** Cortex document id (document submissions). */
  cortex_document_id?: string | null;
  /** Cortex web-import task id (URL submissions). */
  cortex_task_id?: string | null;
...
/**
 * Create a URL (website) submission — no file. On approval it's discovered +
 * crawled into Cortex (see `cortex-management.ts` `crawlIntoCollective`).
 */
export async function createUrlSubmission(input: {
  title: string;
  url: string;
  submitted_by: string;
