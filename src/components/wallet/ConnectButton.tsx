...
import { resetWeb3Session } from "@/lib/web3/reset";
import { AccountSheet } from "./AccountSheet";

// How long "connecting/reconnecting" may persist before we treat the session
// as wedged (a stale WalletConnect pairing can hang the reconnect forever)
// and surface the reset escape hatch instead of a disabled button.
...
export function ConnectButton() {
  ...
  // Stuck-session escape hatch: if wagmi sits in connecting/reconnecting past
  // STUCK_AFTER_MS (stale WalletConnect session — the wallet dropped it but
  // the relay never resolves), re-enable the button as "Reset connection" so
  // users can recover without wiping their whole browser profile.
  const [stuck, setStuck] = useState(false);
  const [resetting, setResetting] = useState(false);
  const stuckTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  useEffect(() => {
    if (isLoading) {
      stuckTimer.current = setTimeout(() => setStuck(true), STUCK_AFTER_MS);
    } else {
      setStuck(false);
    }
    return () => {
  ...
  const handleReset = async () => {
    setResetting(true);
    await resetWeb3Session(disconnect);
  };
  ...
  // Connection wedged — offer the recovery action instead of a dead button.
  if (loading && stuck) {
    return (
      <button
        type="button"
        ...
        className={base}
        style={{ borderColor: "var(--border)", color: "var(--fg1)" }}
        title="The wallet session appears stuck. Click to clear it and start fresh."
      >
        <WalletIcon />
