...
// Public IPFS gateway for client-side fallbacks. The configured gateway stays
// primary (resolveMediaUrl); this one steps in when a thumbnail errors — the
// primary gateway can hang for minutes on CIDs it hasn't pinned, while public
// gateways resolve them via the DHT in well under a second.
const PUBLIC_IPFS_GATEWAY =
  process.env.NEXT_PUBLIC_IPFS_FALLBACK_GATEWAY ||
  process.env.IPFS_FALLBACK_GATEWAY ||
  "https://ipfs.io/ipfs/";

/**
 * Rewrite an ipfs:// URI or any IPFS-gateway URL onto a public gateway.
 * Returns null for non-IPFS URLs.
 */
export function publicIpfsUrl(url?: string): string | null {
  if (!url) return null;
  if (url.startsWith("ipfs://")) {
    return PUBLIC_IPFS_GATEWAY + url.slice("ipfs://".length).replace(/^ipfs\//, "");
  }
  const m = url.match(/^https?:\/\/[^/]+\/ipfs\/([^?#]+)/);
  return m ? PUBLIC_IPFS_GATEWAY + m[1] : null;
}
...
export interface NftView {
  ...
  /**
   * Absolute Directus asset URL (no params) of the museum-cached still
   * (`nfts.image_file`, backfilled by apps/migration/cache-nft-stills.ts).
   * First choice for thumbs and 3D textures — served by our own Directus
   * with on-demand transforms and CORS, no external proxy involved. Null
   * for works not yet cached (e.g. video-only pieces, Multipass imports).
   */
  still?: string | null;
  isVideo: boolean;
  /**
...
export function artworkTextureUrl(view: NftView, size = 1024): string {
  // Museum-cached still: Directus transforms + CORS, no proxy hop at all.
  if (view.still) return `${view.still}?width=${size}&quality=82&format=webp`;
  const still =
...
/** Directus asset base URL for a cached still (`nfts.image_file`). */
function directusAssetUrl(fileId: string): string {
  const base = process.env.DIRECTUS_URL || "https://api.moca.qwellco.de";
...
export function toNftView(nft: {
  ...
  return {
    ...
    preview,
    display,
    still: nft.image_file ? directusAssetUrl(nft.image_file) : null,
