...
> **Provenance.** This repo was extracted from the private MOCA monorepo and is the
> standalone home of `museumofcryptoart.com`. References to `apps/*` paths
> (`apps/api` — the Directus backend, `apps/migration` — offline bake/backfill
> tools, `apps/hyperfy` — the CLI spawner twins) point at tooling that still lives
> in that private monorepo, not in this repository.
...
a **SIWE session** (not the old account system) and stores the review queue in
MOCA's **Directus** backend (not an app-local DB). The anonymous read-only chat is
unchanged.
...
`POST /api/library/submissions` (session-gated, multipart) creates a `pending` row with
the verified `submitted_by` — it does **not** touch Cortex. Two kinds, told apart by
`submission_type`:
- **document** — a `file` multipart part → uploaded to Directus (`createSubmission`).
- **url** — a `url` form field (http/https, ≤2048 chars, no file) → stored as-is
  (`createUrlSubmission`); on approval the whole site is crawled into Cortex.
`GET` (admin) lists for the review table; `[id]/file` streams a file for preview
(admin, keeps the token server-side; 404 for URL rows, which have no file).

**Approve → Cortex.** `POST /api/library/submissions/approve` (admin) promotes each row
into the Cortex **Collective** collection via the **management key**
(`CORTEX_MANAGEMENT_API_KEY`, a `cortex_rw_` key with `manage` permission — distinct
from the read-only chat key), serialized ~1.1 s apart for Cortex's ~1 rps
(`lib/library/cortex-management.ts` resolves-or-creates the collection). Per kind:
- **document** — `POST /api/upload`, then flip to `approved` with the returned
  `cortex_document_id`.
- **url** — `crawlIntoCollective()` branches on the row's `crawl_mode`:
  - **`page`** (default — a single article/interview): imports **only** the submitted
    URL, no discovery. Prevents an article on a large third-party domain (a magazine,
    gallery, news outlet) from dragging in ~100 unrelated same-host pages.
  - **`site`** (a personal website): `POST /api/web-import/discover` (same-host link
    discovery), then the submitted page + discovered links (submitted page first,
    deduped, capped by `CORTEX_CRAWL_MAX_URLS`, default 100).
  Either way `POST /api/web-import` (`content_filter: fit`, `submitted_by` for
  `community:<addr>` provenance) — retrying once truncated if Cortex returns its
  per-job cap 400. Flips to `approved` and stores the async import `cortex_task_id`.
  Legacy rows with no `crawl_mode` fall back to `site`.
  The crawl+ingest runs in Cortex, which **aggregates the site's pages into one
  document per domain** (so subpages read as one related work — an artist named only
  on the About page ties to their practice/exhibitions pages). Its web-import task
  completes once the document(s) are staged, then hands the extract/embed pass to a
  `batch_processing` task; inspect progress in the Cortex Documents UI. Requires `ENABLE_WEB_CRAWL` + a crawl4ai service on the
  Cortex side (a 404 from web-import → clear reviewer message).
A per-item failure leaves the row `pending` for retry. `POST .../reject` marks
`rejected` (kept for audit). Rejected/pending content is never ingested.
...
`SubmitDocumentDialog.tsx` — a **Document / Website** toggle (file dropzone vs. a URL
input). In Website mode a scope selector chooses **A single article or interview**
(default → `crawl_mode: page`) or **My personal website** (→ `crawl_mode: site`);
picking the personal-website option reveals a short notice that whole-site crawls of
third-party articles are declined in review. The moderation table is `/library/review`
...
Approve/Reject on the pending view; URL rows show the site + a **Visit** link (type
`WEB · single page` / `WEB · whole site` so the reviewer sees the scope), document rows
the filename + **Open** preview. `MySubmissions.tsx` (account
sidebar) shows the URL for website rows. Allowed document types mirror Cortex: `.pdf
.txt .md .docx .xlsx`, ≤50 MB.

**Directus setup (operator, one-time).** Create a `library_submissions` collection with
fields: `submission_type` (string: document/url, default `document`), `title` (string),
`file` (M2O → `directus_files`, **nullable** — URL rows have none), `filename`,
`file_type` (string), `file_size` (integer), `url` (string, nullable — the submitted
website), `crawl_mode` (string: site/page, nullable — URL scope; default `page`),
`submitted_by` (string), `status` (string: pending/approved/rejected, default
pending), `rejected_reason` (string, nullable), `cortex_document_id` (string, nullable),
`cortex_task_id` (string, nullable — the web-import task id for URL rows),
`cortex_synced` (boolean, default false), `reviewed_by` (string, nullable),
`reviewed_at` (timestamp, nullable). Then create a dedicated role/policy with CRUD on
`library_submissions` + file create/read, generate a static token →
`DIRECTUS_SUBMISSIONS_TOKEN`. Capture the collection via `npx directus-sync pull` and
commit. Do **not** give this collection a public read policy.
(Legacy rows created before `submission_type` existed have no value → treated as
`document`.)

## Personal Library API keys (account sidebar)

Holders can mint a **personal read-only Cortex key** from the account sidebar
(`components/wallet/LibraryKeyCard.tsx`, rendered in `AccountSheet`): a brief
explainer + one button — Generate, or Rotate behind an inline "your current key
stops working immediately" confirm — then a **one-time reveal** of the secret
(with the `X-API-Key` usage hint). Gate: ≥100 $MOCA, or ≥1 Art DeCC0 / MOCA
ROOM (`isEligibleToSubmit`) — keys only; document submissions are NOT
holdings-gated (any connected wallet may submit, admin review is the filter).

**No database — Cortex's key store IS the registry.** Every issued key is named
`moca-holder:0x<address>` (lowercase) and created with `["read"]` permission via
the Cortex admin key-CRUD API (`lib/library/holder-keys.ts`), using a dedicated
**admin-permission key** `CORTEX_ADMIN_API_KEY` (distinct from both the public
read key and the write-only `CORTEX_MANAGEMENT_API_KEY`, so the ingest key still
can't mint keys). One key per wallet: issuing **deletes** any previous keys for
the address first — that's rotation. Routes (`/api/keys`, session-gated, address
always from the SIWE session): `GET` → status (masked prefix, active flag —
never the secret), `POST` → holdings-gate re-check server-side, then
issue/rotate and return the secret once.

**Holdings sweep (the "orchestrator").** `recheckAllHolderKeys()` lists all
`moca-holder:*` keys, re-reads each wallet's holdings (3 at a time, gentle on
Moralis), **revokes** keys whose wallet dropped below the gate and
**reactivates** them when it qualifies again. Fail-safe: a wallet whose
holdings read throws is skipped, never revoked (and since `getHoldings` fails
soft per source, the auto-reactivate on the next healthy sweep is the recovery
for a false "holds nothing" during an outage). Runs in-process every
`HOLDER_KEYS_RECHECK_HOURS` (default 6, 0 = off; scheduled in
`instrumentation.ts`), and/or externally via `POST /api/keys/recheck` with
`Authorization: Bearer $HOLDER_KEYS_CRON_SECRET` (library admins with a SIWE
session may trigger it too).
...
**Backend error contract (Cortex 2026-07 hardening).** Two 429 flavors, told apart by
the `Retry-After` horizon: the per-key burst limit (seconds) and the monthly unit
quota `MAX_QUERIES_PER_MONTH` (seconds until the next UTC month). `rateLimitMessage()`
(`src/lib/rate-limit-message.ts`) shows `quotaExhausted` with the reset date when the
horizon exceeds 6h; never auto-retry a 429. The submission-approve upload can now also
get **507 Insufficient Storage** (Cortex free-disk guardrail, `MIN_FREE_DISK_MB`) and
**413** (request-body ceilings) — `uploadToCollective` maps 507/429 to clear
...
- **`ArtworkPicker.tsx`** — exports `ArtworkBrowser`, the embeddable search/browse
  ...
  by `MultipassImporter` so both sources render identical cards. **Card media
  walks a fallback chain on load errors** (`thumbSources` + the `ThumbMedia`
  component): transform-in proxy → public IPFS gateway → direct source URL. The
  proxy is a single external point of failure (it went down 2026-07 serving 503
  "no available server" and blanked every card), and the primary
  `ipfs.qwellcode.de` gateway can hang minutes on CIDs it hasn't pinned — so a
  proxy outage must degrade to unoptimized originals, never broken images.
...
- **`SpawnHyperfyDialog.tsx`** — spawns the current exhibition into any
  ...
  **`GET /v1/guide/followup?exhibition&session`** — it *extends* the fast answer
  ...
  chunk; `speak`/`voice` are app inspector props). That endpoint lives in the MOCA
  Directus backend (`apps/api` in the private monorepo, `src/v1/guide.ts`)
  ...
  **`GuideDialog.tsx`** (sidebar Exhibits tab → "Museum guide") is the
  ...
  `protocol.ts`/`room-script.ts`/`guide-script.ts`/`hyp.ts` are **twins of the
  CLI spawner's `apps/hyperfy/lib/*.mjs`** (private monorepo) — keep them in sync.
...
**Texture loading (don't regress this).** First choice for stills is the
**museum-cached Directus asset** (`nfts.image_file`, backfilled by
`apps/migration/cache-nft-stills.ts` — surfaces as `NftView.still`, an absolute
`/assets/<id>` URL): our own Directus serves on-demand transforms with CORS
reflected for any origin, so `artworkTextureUrl` and the picker's `thumbSources`
use it with zero external dependencies (this also feeds Hyperfy spawns). Works
without a cached still fall back to the same-origin route
**`/api/museum/texture?src=…&w=…`**, never directly to media hosts:
`THREE.TextureLoader` needs CORS on *every* redirect hop, and the
...
(WebP resize + dead-URL revival) with a public-IPFS-gateway and direct-origin
fallback, blocks SSRF targets, and caches hard. Motion-only works (no still poster) hang as live
...
- **Community submissions (optional — feature is disabled if unset):** `SESSION_SECRET`
  ...
  (target collection name, default `Collective`), `CORTEX_CRAWL_MAX_URLS` (soft cap on
  pages crawled per URL submission, default 100). Uses the existing `DIRECTUS_URL` +
  mainnet RPC (for ENS). URL submissions additionally need `ENABLE_WEB_CRAWL` + a
  crawl4ai service configured on the **Cortex** deployment. See "Community submissions
  & review" above.
