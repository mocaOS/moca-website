...
// Within this distance (px) of the bottom the view counts as "at the end";
// further up, the jump-to-latest button is shown.
...
export default function MessageList({
  ...
  const updateJump = useCallback(() => {
    ...
    if (!el) return;
    setShowJump(el.scrollHeight - el.scrollTop - el.clientHeight >= BOTTOM_THRESHOLD);
  }, []);
  ...
  // Streaming never moves the view — the reader follows the answer at their
  // own pace. Content growth only decides whether the jump button shows.
  const lastMessageId = messages.length > 0 ? messages[messages.length - 1].id : null;
  useEffect(() => {
    updateJump();
  }, [messages, updateJump]);
  // A genuinely new message (user just sent) scrolls once to the start of
  // the new turn.
  useEffect(() => {
