        ...
        await askQuestionStream(
          request,
          {
            ...
            onRetrieval: (info: string) => {
              // Zero-result lines ("Found 0 relevant communities") add no
              // information and read as if the search came back empty — keep the
              // current retrieval line instead. Counts only ever grow.
              if (/^Found 0\b/i.test(info)) return;
              setMessages((prev) =>
                prev.map((m) =>
                  m.id === assistantId
            ...
            onStatus: (status) => {
              ...
              // Zero-result labels ("Found 0 relevant communities") confuse more
              // than they inform — keep the previous label. (Once a stage has a
              // running total, aggregation has already rewritten the 0 to that
              // total, so this only fires while the count is genuinely zero.)
