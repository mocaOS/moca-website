...
/**
 * Approve selected submissions into the Cortex "Collective" collection via the
 * management key, then flip the row to `approved`. Admin only. Two kinds:
 *  - **document** → upload the file (`POST /api/upload`).
 *  - **url** → discover same-host links + web-import the whole site
 *    (`POST /api/web-import`); the async import task id is stored on the row and
 *    processing continues (and is inspectable) in Cortex.
 * Processed serially (~1.1 s apart) to respect Cortex's ~1 rps limit; a per-item
 * failure leaves that row `pending` so it can be retried.
 */
export async function POST(req: NextRequest) {
  ...
  for (const id of parsed.data.ids) {
    try {
      ...
      const isUrl = sub.submission_type === "url" || (!!sub.url && !sub.file);

      // Respect Cortex's ~1 rps between actual write actions.
      if (processed > 0) await sleep(1100);
      processed++;

      if (isUrl) {
        if (!sub.url) {
          results.push({ id, ok: false, error: "Missing URL" });
          processed--;
          continue;
        }
        const { taskId } = await crawlIntoCollective({
          url: sub.url,
          submittedBy: sub.submitted_by || undefined,
          mode: sub.crawl_mode ?? undefined,
        });
        await updateSubmission(id, {
          status: "approved",
          cortex_task_id: taskId,
          cortex_document_id: null,
          cortex_synced: true,
