...
import { clearWeb3Storage } from "@/lib/web3/reset";
import { LibraryKeyCard } from "@/components/wallet/LibraryKeyCard";
import { MySubmissions } from "@/components/wallet/MySubmissions";
...
export function AccountSheet({
  ...
  // Top-level sheet sections: Holdings (balances, NFTs, staking recovery) and
  // Community (library submissions + personal API key / agent setup).
  const [tab, setTab] = useState<"holdings" | "community">("holdings");
  ...
  const handleDisconnect = async () => {
    ...
    await signOut().catch(() => {});
    // A dead WalletConnect session can hang or reject the graceful disconnect.
    // Bound it, and fall back to hard-clearing all persisted wallet state so
    // logging out always works.
    const OK = Symbol("disconnected");
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        >
          {(["holdings", "community"] as const).map((t) => {
            const on = t === tab;
            return (
              <button
                key={t}
        ...
        {tab === "holdings" ? (
          ...
          <div className="flex flex-col gap-5">
            <MySubmissions />
            {/* Key management needs the holdings (eligibility gate), so it
                appears once they're in — the loading/error states above cover
                the wait. */}
            {data && <LibraryKeyCard holdings={data} />}
          </div>
        )}
      </aside>
    </div>,
...
function CollectionsTabs({
  ...
  // 3-column grid × 3 rows max, so the sidebar shows at most 9 NFTs per page
  // and the staking-recovery link below stays visible without scrolling.
  const PAGE_SIZE = 9;
