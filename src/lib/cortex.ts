...
// An admin-permission Cortex key used ONLY to mint/revoke the per-holder
// read-only "Library API keys" (see lib/library/holder-keys.ts). Key CRUD needs
// the `admin` permission, which the write-only management key above doesn't
// carry — keep them separate so the submission-ingest key can't mint keys.
// Returns null when unset — holder keys then report "not configured".
export function getCortexAdminKey(): string | null {
  return process.env.CORTEX_ADMIN_API_KEY || null;
}
