...
// "Enter Library" CTA. Vibe Studio lives in the footer (and on /decc0s).
const NAV = [
...
export default function SiteHeader({ logoUrl = "/logo.svg" }: { logoUrl?: string }) {
  ...
  const renderDesktopItem = (item: (typeof NAV)[number]) => (
    <Link
      key={item.href}
      href={item.href}
      className="relative whitespace-nowrap rounded-[var(--radius)] px-3.5 py-2 text-sm transition-colors"
    ...
    >
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-8">
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          ...
          <Link
            href="/library"
            className="ml-2 flex h-9 shrink-0 items-center whitespace-nowrap rounded-[var(--radius)] px-4 text-sm font-medium transition-transform active:scale-[0.98]"
        ...
        <button
          onClick={() => setOpen((v) => !v)}
          className="flex h-10 w-10 items-center justify-center rounded-[var(--radius)] lg:hidden"
          style={{ color: "var(--fg1)" }}
          aria-label="Menu"
        >
      ...
      {open && (
        ...
        >
          ...
          <div
            className="mt-3 flex items-center gap-2 border-t px-3 pt-4"
            style={{ borderColor: "var(--border)" }}
          >
            <Link
              href="/library"
              onClick={() => setOpen(false)}
              className="flex h-9 flex-1 items-center justify-center rounded-[var(--radius)] px-4 text-sm font-medium transition-transform active:scale-[0.98]"
              style={{ background: "var(--accent)", color: "var(--accent-fg)" }}
            >
              Enter Library
            </Link>
