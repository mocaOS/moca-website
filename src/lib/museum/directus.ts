...
export interface Nft {
  ...
  /**
   * Museum-cached still image (directus_files id), backfilled by
   * apps/migration/cache-nft-stills.ts. Preferred over external media hosts
   * for thumbs/textures; null until the backfill has covered the work.
   */
...
export interface Room {
  ...
  /**
   * Baked slot anchors with resolved facing, computed from the room's GLB by
   * apps/migration/bake-slot-data.ts (shape: RoomSlotData in
   * components/museum/three/slots.ts). Publicly readable; also served at
   * /v1/rooms/:id/slots on the MOCA API.
   */
  slot_data?: unknown;
...
export const listNfts = unstable_cache(
  async (opts: {
    ...
    const fetchNfts = (extras: string[]) =>
      client().request(
        readItems("nfts", {
          fields: [ ...NFT_FIELDS, ...extras ],
    ...
    let rows;
    try {
      // image_file (cache-nft-stills.ts) is created by the migration's first
      // --write run — fall back gracefully on instances that predate it.
      rows = await fetchNfts([ "image_file" ]);
    } catch {
      rows = await fetchNfts([]);
    }
...
export const listRooms = unstable_cache(
  async (): Promise<Room[]> => {
    ...
    const fetchRooms = (extras: string[]) =>
      client().request(
        readItems("rooms", {
          // model_optimized (embed-room-slots.ts) and slot_data
          // (bake-slot-data.ts) are created by their script's first --write
          // run — fall back gracefully on instances that predate them.
          fields: [ ...ROOM_FIELDS, ...extras ],
          sort: [ "series", "title" ],
          limit: -1,
        }),
      );
