...
import { z } from "zod";
import { proxiedUrl, publicIpfsUrl } from "@/lib/museum/media";
...
export async function GET(req: NextRequest) {
  ...
  // Optimizing proxy first (WebP resize + dead-host revival). For IPFS-hosted
  // works try a public gateway before the configured one — the configured
  // gateway can hang on CIDs it hasn't pinned (fetchImage's 15s timeout turns
  // that into a slow 502), while public gateways resolve them in under a second.
  const publicIpfs = publicIpfsUrl(url.href);
  const upstream =
    (await fetchImage(proxiedUrl(url.href, { width: w, format: "webp", q: 82 }))) ??
    (publicIpfs ? await fetchImage(publicIpfs) : null) ??
